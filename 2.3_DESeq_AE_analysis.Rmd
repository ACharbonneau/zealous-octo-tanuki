...
```{r PseudoInteractions DESeq2, results='hide', include=FALSE}
...
rld <- rlog(raphpseudo.DES, blind = FALSE)
sampleDists <- dist(t(assay(rld)))
df <- as.data.frame(colData(raphpseudo.DES)[,c("Line","Location")])
...
#### High vs low, no interaction term, adjusted p value <.1 and a FC of at least `r FC_cutoff` 
...
#### HighKBS vs lowKBS, pseudointeraction term, adjusted p value <.1 and a FC of at least `r FC_cutoff`
```{r}
...
#### HighReed vs lowReed, pseudointeraction term, adjusted p value <.1 and a FC of at least `r FC_cutoff` 
```{r}
...
For illustration purposes, here are plots of the modeled counts for the `r length(results_hvl_11_Ordered$baseMean)` genes shown in the summary for High vs low with no interaction term, an adjusted p value <.1 and a FC of at least `r FC_cutoff`, ordered by adjusted pvalue: 
...
abline(h=c(-FC_cutoff,FC_cutoff), col="dodgerblue", lwd=2)


```

Blue lines at `r FC_cutoff` fold change
...
#### KBS vs Reed, no interaction term, adjusted p value <.1 and a FC of at least `r FC_cutoff`
...
#### HighKBS vs HighReed, pseudointeraction term, adjusted p value <.1 and a FC of at least `r FC_cutoff`
```{r}
summary(hKvhR_kvr_11)
```


#### LowKBS vs LowReed, pseudointeraction term, adjusted p value <.1 and a FC of at least `r FC_cutoff` 
```{r}
