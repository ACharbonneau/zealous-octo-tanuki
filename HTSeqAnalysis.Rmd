...
outputfilenamePseudoHvL <- ("../DEseqOutput/Mitsui2015_deseq2_expr_changes_HvL.csv")
outputfilenamePseudoKvR <- ("../DEseqOutput/Mitsui2015_deseq2_expr_changes_KvR.csv")
...
```{r GlobalTranscriptome, echo=FALSE, eval=FALSE}
...
no_feature <- as.data.frame(ALLTHEGENES[ length( rownames( ALLTHEGENES )) - 4,])
...
count <- as.data.frame(colSums(ALLTHEGENES[2:(File_Num+1)]))

counts <- cbind(count, t(no_feature)[2:36])

write.csv(counts, outputfilenameCounts)
...
Presults_hRvhK <- results( raphpseudo.DES, contrast=c("LocationLine", "ReedHigh", "KBSHigh"), cooksCutoff=FALSE, independentFiltering=TRUE, addMLE=TRUE, pAdjustMethod="BH", alpha = p.threshold)
...
## contrast=c(column, numerator, denominator)
Intresults_hvl <- results( raphInt.DES, contrast=c("Line", "High", "Low"), cooksCutoff=FALSE, independentFiltering=TRUE, # addMLE=TRUE, Error in results(raphInt.DES, contrast = c("Line", "High", "Low"), cooksCutoff = FALSE, addMLE=TRUE is only for when a beta prior was used. otherwise, the log2 fold changes are already MLE 
...
This model found `r sum(Presults_hvl$padj < p.threshold, na.rm = TRUE)` differentially expressed genes with a p value < .05

####Summary Information from model and graph of dispersion shrinkage
```{r PseudoSummaryplots, echo=FALSE}
#Summary of modeled values that meet the specified adjusted p-value cutoff
summary( Presults_hvl, alpha=p.threshold)
...
plotMA( Presults_hvl, MLE=TRUE, main="unshrunken LFC", ylim=c(-3,3), alpha=p.threshold )

plotMA( Presults_hvl, MLE=FALSE, main="Shrunken LFC", ylim=c(-3,3), alpha=p.threshold  )
...
ggplot(results_hvl_df, aes(ABlog2FoldChange, padj)) + geom_point(aes(col=log(baseMean)))

ggplot(results_hvl_df, aes(log2FoldChange, padj)) + geom_point(aes(col=log(baseMean)))

ggplot(results_hvl_df, aes(log2FoldChange, log10(baseMean))) + geom_point(aes(col=padj))
...
Presults_hRvlR_df <- filter(Presults_hRvlR_df, !is.na(log2FoldChange))
Presults_hKvlK_df <- filter(Presults_hKvlK_df, !is.na(log2FoldChange))
...
write.csv( Presults_hvl_df, outputfilenamePseudoHvL, quote=FALSE, row.names=FALSE)

plot_grid(
ggplot(Presults_hvl_df, aes(log2FoldChange.x, baseMean.x, label=gene_name)) +
    geom_text_repel(data=filter( Presults_hvl_df, baseMean.x > 30), aes(col=padj.x)) +
    geom_point(aes(col=padj.y)) + 
    labs(title = "High vs Low Reed", x="Log 2 Fold Change", y="Base Mean") ,

ggplot(Presults_hvl_df, aes(log2FoldChange.y, baseMean.y, label=gene_name)) +
    geom_text_repel(data=filter( Presults_hvl_df, baseMean.x > 30), aes(col=padj.y)) +
    geom_point(aes(col=padj.y)) + 
...
write.csv( Presults_RvK_df, outputfilenamePseudoKvR, quote=FALSE, row.names=FALSE)

plot_grid(
ggplot(Presults_RvK_df, aes(log2FoldChange.x, baseMean.x, label=gene_name)) +
    geom_text_repel(data=filter( Presults_RvK_df, baseMean.x > 30), aes(col=padj.x)) +
    geom_point(aes(col=padj.y)) + 
...
miniresults <- cbind( 
  head(results_hvl_df[, c("gene_name", "log2FoldChange", "padj", "MLE")], 10), 
  head(Intresults_hvl_df[, c("gene_name", "log2FoldChange", "padj")], 10),
  head(Presults_hvl_df[, c("gene_name", "log2FoldChange", "padj")], 10))
...
Comparison of top 10 highest adjusted p-value genes from "No interaction" model (top) versus the interaction, but no shrinkage model (bottom). Only one of the top 10 genes, `r intersect(miniresults[,1], miniresults[,5])`, are in all lists.


```{r CompareGeneLists, echo=FALSE}
tempresults <- cbind( head(results_hvl_df[, c("gene_name", "log2FoldChange", "padj")], 100), 
               head(Intresults_hvl_df[, c("gene_name", "log2FoldChange", "padj")], 100), 
               head(Presults_hvl_df[, c("gene_name", "log2FoldChange", "padj")], 100))

```

In the top 100 highest adjusted p-value genes, there are just `r length(intersect( tempresults[,1], tempresults[,4]))` genes that appear in all three:
