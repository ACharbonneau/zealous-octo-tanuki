...
if ( sum(no_feature) != 0 ) { 
genecounts <- cbind(genecounts, t(no_feature)[2:36])
...
```{r, echo=FALSE, fig.path=paste("../figures/",subgroup$Dataname,"/", sep=""), fig.keep="last", fig.width=11}
...
dplyr::select(Presults_hvl_df, gene_name, log2FoldChange.x, padj.x) %>% arrange(padj.x) %>% head( n=10 )
