...
```{r PlainGeneList, echo=FALSE, results='hide', fig.path="../figures/", fig.keep="last", fig.width=11}
...
```{r, echo=FALSE, fig.path="../figures/", fig.keep="last", fig.width=11}
ggplot(Presults_hvl_df, aes(log2FoldChange.x, baseMean.x, label=gene_name)) +
    geom_text_repel(data=filter( Presults_hvl_df, baseMean.x > 20 & log2FoldChange.x > 2 ), aes(col=padj.x)) +
    geom_point(aes(col=padj.x)) + 
...
ggplot(Presults_hvl_df, aes(log2FoldChange.y, baseMean.y, label=gene_name)) +
    geom_text_repel(data=filter( Presults_hvl_df, baseMean.y > 20 & log2FoldChange.y > 2), aes(col=padj.y)) +
...
```{r, echo=FALSE}
select(results_hvl_df, gene_name, log2FoldChange, padj) %>% arrange(padj) %>% head(n=10)
...
select(Intresults_hvl_df, gene_name, log2FoldChange, padj) %>% arrange(padj) %>% head(n=10)
...
select(Presults_hvl_df, gene_name, log2FoldChange.x, padj.x) %>% arrange(padj.x) %>% head( n=10 )
