...
#Also have to change gene number indexing on line 251 or the lda won't run
...
ALLTHEGENES <- filter(ALLTHEGENES, grepl("gene", Genes))
...
raphInt.DES <- DESeqDataSetFromMatrix(countData = exprs( raph.est ), colData = pData( raph.est ), design = ~ Location:Line + FlowCell + Location + Line )
...
results_hvl <- results( raph.DES, contrast=c("Line", "High", "Low"), cooksCutoff=FALSE, independentFiltering=FALSE, addMLE=TRUE)
...
## contrast=c(column, numerator, denominator)
Intresults_hvl <- results( raphInt.DES, contrast=c("Line", "High", "Low"), cooksCutoff=FALSE, independentFiltering=FALSE)
...
##Top 50 heatmap
Heatmap of modeled expression levels of 50 most highly expressed genes in all individuals. 
...
```{r MiniPlainLineHeatmap, echo=FALSE}

select50 <- order(rowMeans(counts(raph.DES,normalized=TRUE)),decreasing=TRUE)[1:50]

pheatmap(assay(vsd)[select50,], cluster_rows=F, show_rownames=F, cluster_cols=F, annotation_col=df)

```

##Clustered Top 50 heatmap

Clustering that same heatmap does show some structure, however read counts seem to be much more correlated with Location than with Line. In particular, the upper left corner with a tight cluster of down-regulated genes is four plants from Reed, but has both phenotypes.

```{r MiniPlainClusterLineHeatmap, echo=FALSE}

pheatmap(assay(vsd)[select50,], cluster_rows=T, show_rownames=FALSE, cluster_cols=T, annotation_col=df)

```

##Sample vs Sample heatmap

This graph sample heatmap echos the gene heatmaps in that there is little Low vs High structure to cluster out individuals by. This map is clustered, however the clusters are not very strong.

```{r PlainSampleHeatmap, echo=FALSE}
...
First two components of a PCA of the same normalized distance matrix used in the previous heatmap. Its obvious that the eigenvectors are not directly capturing High vs Low, however it appears that PC1 and PC2 are partially clustering KBS vs Reed.

```{r plainPCA, echo=FALSE}
vsd_df <- as.data.frame(t(assay(vsd)[select,]))
df$name <- rownames(df)

plotPCA(vsd, intgroup=c("Line", "Location"))
...
plotMA( results_hvl, MLE=TRUE, main="unshrunken LFC", ylim=c(-2,2) )

plotMA( results_hvl, MLE=FALSE, main="Shrunken LFC", ylim=c(-2,2) )
...
plotMA( Intresults_hvl, main="unshrunken LFC", ylim=c(-2,2) )
...
results_hvl_df <- data.frame( results_hvl )
...
miniresults <- cbind( head(results_hvl_df[, c("gene_name", "log2FoldChange", "padj")], 10), head(Intresults_hvl_df[, c("gene_name", "log2FoldChange", "padj")], 10))

miniresults

#intersect(miniresults[,1], miniresults[,4])

```

Comparison of top 10 highest adjusted p-value genes from "No interaction" model (top) versus the interaction, but no shrinkage model (bottom). Only one of the top 10 genes, `r intersect(miniresults[,1], miniresults[,4])`, is in both lists.
...
In the top 100 highest adjusted p-value genes, there are just `r length(intersect( tempresults[,1], tempresults[,4]))`:
...
deseq2_results_rvk <- results( raph.DES, contrast=list( "LocationReed", "LocationKBS" ), cooksCutoff=FALSE, independentFiltering=FALSE )
